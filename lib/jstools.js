...
function qq(log){
  ...
  // stop dirty debugging if in demoMode
  if(!GLB.demoMode){
...
function nestedElasticResultsGroupedByField(obj, results, path){
  ...
  _.each(obj, function(obj2, key2){
    ...
    qqq && qq(results)
    qqq && qq("checking \""+key2+"\"")

    if($.isArray(obj2)){
      qqq && qq("==array, adding all to list")
      if(!results.hasOwnProperty[key2]){
        path2 = JSON.parse(JSON.stringify(path))
        path2.push(key2)
        results[path2.join(".")] = []
      }
      
      _.each(obj2, function(element){
        path2 = JSON.parse(JSON.stringify(path))
        path2.push(key2)
        results[path2.join(".")].push(element)
      })
      qqq && qq("CANT CLICK: "+key2)
      return results     

    }else if(typeof(obj2) == 'object'){
      qqq && qq("==object, go deeper")
      
      path2 = JSON.parse(JSON.stringify(path))
			path2.push(key2) 
      nestedElasticResultsGroupedByField(obj2, results, path2)
    
    }else{
      
      qqq && qq(key2+" not object, adding to list")
      
      if(!results.hasOwnProperty[key2] || key2 == "name"){
        // qqq && qq("creating keyzzz "+"\""+key2+"\"")
        // results[path.join(".")+"."+key2] = []
        path2 = JSON.parse(JSON.stringify(path))
        path2.push(key2)
        results[path2.join(".")] = []
      }
      ...
      qqq && qq(results)
      // results[path.join(".")+"."+key2].push(obj2)
      // return results
      path2 = JSON.parse(JSON.stringify(path))
      path2.push(key2)
      results[path2.join(".")].push(obj2)
      return results
