...
var graphs_functions_json = {
	...
	retrieveGraphParam: function(type, graphshort, value){
		...
		}else if(this.local_json[type] && this.local_json[type][graphshort] && typeof this.local_json[type][graphshort][value] !== null){
	...
	graphTypeBelongTo: function(graphType, belongTo){


		if(this.local_json.hasOwnProperty(belongTo) && this.local_json[belongTo].hasOwnProperty(graphType)){
			return true
		}else{
			return false
		}
...
async function drawinBoxes(ids){
	...
	for (var i in ids){
		...
		// builtin / introduction squares don't need times calculating
		var thisType = await nameToConnectorAttribute(retrieveSquareParam(id, 'Co', true), "type")	
		if(thisType != "builtin" && thisType != "introduction"){
			var thisDate = new Date();
			var thisEpoch = Math.floor(thisDate.getTime() / 1000); 
			// keep times in epoch, but adjut for this user, no need to apply our timezone offset agaisnt thisWe, as moment already does that for us
			var thisWe = calcGraphTime(id);
			var thisWs = retrieveSquareParam(id, "Ws", true);
			
			if(thisWe === null){
				graphGraphError(id, "Square does not have 'Window End' set "+thisWe)
				continue
			}else if(thisWs === null){
				graphGraphError(id, "Square does not have 'Window Size' set")
				continue
			}
		...
		if(theResult && theData){
			// a real graph, but it appears we have vaild data
			qq("id:"+id+" == data valid")

			callTheGraph(id, true);
	
		}else{
			// a real graph, and needs data. 
			qq("id:"+id+" == get fresh data")

			var thisName = retrieveSquareParam(id, 'Co', true)
			var thisDst = await nameToConnectorAttribute(thisName, "dst")
			var thisGT = retrieveSquareParam(id, "Gt")

			// preflight checks
			if(thisName === undefined){
				graphGraphError(id, "Root connector does not have a 'Connector Name' set")
				continue
			
			}else if(thisType != "builtin" && thisType != "introduction" && elasticPreFlightCheck(id) == false){
				graphGraphError(id, "Error connecting to "+thisDst+".<br>- Check network<br>- Check Firewall<br>- Check ElasticSearch CORS enabled")
				continue
			}
			...
			// "await" is sync, promise.then is async/parallel which is perfect for multiple parallel CORS requests
			var thisPopulateGraph = graphs_functions_json.retrieveGraphParam(thisType, thisGT , "populate")
			
			if(!thisPopulateGraph){
				graphGraphError(id, "Misconfiguration: This graph type does not belong to this Connector.  Please edit and resave.")
				continue
			}

			window[thisPopulateGraph](id)
			.then(async function(values){
				...
				//recalculate as this is now in a callback 
				var thisName = retrieveSquareParam(id, 'Co', true)
				...
				// 	graphGraphError(thisId, "Data Processing error:"+JSON.stringify(values[0].error,null,2))
				// }
				...
				if(dataToProgess){
					...
					// qq("id:"+id+" == no data to process")
					saveNewData(thisId, "processed", "", null)
						.catch(e => ww(0, "saveNewData: id:"+thisId+", error:"+e));		
				}	
...
async function callTheGraph(id, nudgeChildren){
	...
	if(theDynamicGraphFunction != null){
		...
		if(graphs_functions_json.retrieveGraphParam(thisType, retrieveSquareParam(id, "Gt") , "requireThreeJS") == true && GLB.threejs.enabled == false){
			...
			if(graphs_functions_json.graphTypeBelongTo(retrieveSquareParam(id, "Gt"), "builtin") || graphs_functions_json.graphTypeBelongTo(retrieveSquareParam(id, "Gt"), "introduction")){
				qq("id:"+id+" builtin/introduction -> graph()")
				window[theDynamicGraphFunction](id, savedData);
			}else if(savedData != null){
				qq("id:"+id+" data exists -> graph()")
				window[theDynamicGraphFunction](id, savedData);
			}else{
				qq("id:"+id+" emptry resuylts -> graphError()")
...
function getLineCol(id){
	...
	// line colour between graphs signifies the difference in relation

	if(( retrieveSquareParam(id, 'We', false) != null || retrieveSquareParam(id, 'Ws', false) != null) && (retrieveSquareParam(id, 'Ds', false) != undefined || retrieveSquareParam(id, 'Gt', false) != undefined)){
		// lots of changes
		return "line_colour_zero";

	}else if( retrieveSquareParam(id, 'We', false) != null || retrieveSquareParam(id, 'Ws', false) != null){
		// Time not null
...
function saveMappingsData(dst, indexPattern, value){
	...
	return "Mappings for "+_.keys(masterMappings).join(", ")
...
function retrieveSquareParam(id, key, recursive, recursiveCount){
	...
	if(item.hasOwnProperty("Pr") && item.Pr[0] === 0){		
		// root square, make a decissions
		// ww(4, "answer for root square id:"+id+" key:"+key)
		...
		// non recursive query, or something that can't be inherited
		// ww(4, "answer non recursive id:"+id+" key:"+key)
		...
		// ww(4, "answer recursive id:"+id+" key:"+key)
...
function calcGraphTime(id){
	...
	// If I'm a root square, and with null as We, then return null
	if(parents.length == 0 && !retrieveSquareParam(id, "We", false)){
		return null
	}
...
async function editNewConnector(id){
	...
	var thisName = retrieveSquareParam(id, 'Co', false)		
	var thisType = await nameToConnectorAttribute(thisName, "type")		
	if(thisType && thisType != "builtin" && thisType != "introduction"){
		// qq("prepMappings for id:"+id+" type:"+thisType)

		var thisCo = await nameToConnectors(thisName)
		var thisDst = thisCo['dst']			
		var thisIndex = "*"
		// qq("prep for "+id+" "+thisType+" "+JSON.stringify(thisCo))

		var mappings = await elastic_prep_mappings(thisDst, thisIndex, id)		

		saveMappingsData(thisDst, thisIndex, mappings)
...
async function initialLoad(){
	...
	for(var i in tmpEveryID){
		...
		// Look for square that have Co defined (aka root squares)
		var thisName = retrieveSquareParam(id, 'Co', false)		
		var thisType = await nameToConnectorAttribute(thisName, "type")			


		// Only proceed for real Graphs
		if(thisType && thisType != "builtin" && thisType != "introduction" && thisName != "Dummy"){
			// qq("prepMappings for id:"+id+" type:"+thisType)
...
function makeLandingPageLayout(){
	var url = {}
	
	url.v = "1"

	url.squares = []
	url.squares.push({"id":1,"Pr":[0],"Gt":"intro","Wi":[0,0],"x":-800,"y":-800,"Co":"Introduction"})
...
function onLoad(){		
	...
	//	is this a dummy page?  Overwrite config
